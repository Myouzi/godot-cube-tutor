...
2. **A-2 上游许可确认**:**已决策,待用户发送(2026-09-25)**——issue 文本已代拟至 `docs/upstream-license-issue.md`(标题 + 正文 + 发送地址),由项目所有者用 GitHub 账号发送;无果则以社区通用公式集重录替换 `data/cfop.json`。「公式本身不受版权保护,具体编排与呈现方式受」一语**转引自 docs/open-source-comparison.md:160**(调研报告 A-2 行动项原文),系调研报告的判断而非法律意见,行动前应自行核实或咨询。
